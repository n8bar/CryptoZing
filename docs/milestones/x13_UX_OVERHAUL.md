...
## Phase 1. [x] Theme and accessibility

   1. [x] Light/Dark mode toggle %<8549>
   ...
   2. [x] Theme toggle accessibility polish (guardrail follow‑up on Completed Task #2) %<8552>
...
## Phase 2. [x] Public and entry surfaces

   1. [x] Landing page refresh %<8550>
   ...
   4. [x] Public/share refresh (Completed) %<8568>
...
## Phase 3. [x] Wallet and onboarding
   ...
   1. [x] Findings Strategy (Implementation Order) %<8570>
      See [`x13.3_GETTING_STARTED_STRATEGY.md`](../strategies/x13.3_GETTING_STARTED_STRATEGY.md).
   2. [x] Wallet UX improvements %<8554>
      - Wallet settings now include inline guidance and derive validation preview; additional wallets UI is deferred to post-RC.
      1. [x] Onboarding wizard (Completed) %<8569>
      ...
      2. [x] Login redirect to wallet setup (interim bridge) %<8555>
...
## Phase 4. [x] Invoices & Clients UI polish (Completed)
...
- Follow-up closure (2026-02-21): authenticated invoice/client narrower-screen action-row sweep complete; temporary Task 9 ToDo split merged back into this completed task.
   ...
   1. [x] Dashboard snapshot redesign (counts/totals/recent payments). %<8548>
   ...
   2. [x] Human-eyes QA checklist (2026-02-19 working set) %<8557>
      1. [x] Status pills on invoice show are readable in light/dark (`SENT` light-blue with dark text; `PARTIAL` cyan with dark text; `VOID` remains readable in dark mode). %<8558>
      2. [x] Colored cards use matching border/text color treatment (`border-color: currentColor`) on invoice/client/settings surfaces. %<8559>
      3. [x] Draft warning logic only appears for draft invoices with real on-chain payments (not manual-adjustment-only cases). %<8560>
      4. [x] Draft warning CTA uses text-link `Mark sent` and successfully updates status. %<8561>
      5. [x] Invoice show section order is: Payment QR -> Delivery log -> Payment history -> Public link. %<8562>
      6. [x] Public link `Open`/copy actions use the configured host for dev (`http://192.168.68.25/...` when `APP_PUBLIC_URL` is set accordingly). %<8563>
      7. [x] Edit-page public notice copy/link pattern: `Or open invoice details.` with only `open invoice details` linked. %<8564>
      8. [x] Delete cards have visible red borders in light mode on both invoice edit and client edit screens. %<8565>
...
## Phase 5. [x] Invoice settings and invoice UX finish-up (Completed, 2026-03-07)
   ...
   3. [x] Phase B — Implementation %<8599>
      ...
      4. [x] Paid-invoice print polish: render a prominent, translucent diagonal `PAID` watermark on paid print views (owner print and active public print) so payment state is unmistakable in exported/printed %<8603>
      5. [x] Client-facing over/underpayment wording polish: replace generic “invoice sender” phrasing with biller/brand-facing wording (use invoice billing name with a safe fallback) so public/print copy reads %<8604>
      6. [x] Paid-invoice payment-action safety: hide payment QR + BIP21/copy payment action surfaces once an invoice is paid across owner and client views (including owner invoice show, owner print, and active %<8605>
      7. [x] Branding & footer reset affordance: add a clear “Reset to my custom defaults” action near the top of the create/edit Branding & footer section so users can quickly revert per-invoice overrides back %<8606>
      ...
      10. [x] IA correction implementation: move the overpayment gratuity note toggle and QR refresh reminder toggle from Profile UI into Invoice Settings UI, keeping persistence/default behavior user-level for %<8609>
      11. [x] Settings IA shell: add a unified Settings surface with tabs for `Account`, `Wallet`, `Invoice`, and `Notifications` (initial pass can reuse existing forms/routes behind the new tab shell), and keep %<8610>
      ...
      14. [x] Account-menu cleanup: collapse `Account`, `Wallet`, and `Invoice Settings` into one `Settings` entry (default tab: `Account`) so account menu items are only `Settings`, `Getting Started`, and %<8613>
   4. [x] Phase C — Post-implementation Browser QA (acceptance + regression) %<8614>
