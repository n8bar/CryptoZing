...
This document is a temporary working plan. It is not a source of truth like `docs/specs/ONBOARD_SPEC.md`, `docs/milestones/x13_UX_OVERHAUL.md`, `docs/PLAN.md`, `docs/PRODUCT_SPEC.md`, or `docs/CHANGELOG.log`, and may be deleted after Task 11 ships.

## Canonical Inputs
- Product/flow requirements: `docs/specs/ONBOARD_SPEC.md`
- UX milestone tracking: `docs/milestones/x13_UX_OVERHAUL.md`
...
- Browser-QA findings and follow-up UX refinement items now live in:
  - `docs/strategies/x13.3_UX_ENGINEERING_PASS.md`
...
- [x] 1. Orientation entry (Finding 1) %<8571>
...
- [x] 2. Back-link clarity + placement (Finding 2) %<8572>
...
- [x] 3. Wallet helper discoverability (Finding 3) %<8573>
...
- [x] 4. Dashboard prompt controls (Finding 4) %<8574>
...
- [x] 5. Zero-client invoice gate (Finding 5) %<8575>
...
- [x] 6. Guided focus emphasis (Finding 6) %<8576>
...
- [x] 7. Replay mode for completed users (Finding 7) %<8577>
...
- [x] 8. New browser QA pass (coverage + gap finder) %<8578>
...
- Suggested execution order (about 30-45 minutes total):
  - UXInspection A (10-15m): Core regressions
    - [x] New registrant lands on welcome entry and can complete all steps end-to-end. %<8579>
    - [x] Incomplete returning user lands on resolver-first actionable step (not forced to welcome). %<8580>
    - [x] Replay-started user enters wallet step first and does not skip directly to dashboard. %<8581>
  - UXInspection B (10-15m): Replay boundary checks
    - [x] Pre-replay artifacts do not auto-complete replay invoice/deliver steps. %<8582>
    - [x] New replay invoice advances to deliver step. %<8583>
    - [x] Replay completion requires replay-era delivery activity and then closes with completion status. %<8584>
    - [x] Replay state survives logout/login and resumes at the expected step. %<8585>
  - UXInspection C (5-10m): Deliver-step targeting correctness
    - [x] Multiple eligible draft invoices: Change selector swaps target correctly. %<8586>
    - [x] Invalid or stale `?invoice=` falls back safely. %<8587>
    - [x] Non-eligible invoices (sent/void/trashed/other-user) cannot become target invoice. %<8588>
  - UXInspection D (5-10m): UX/a11y quick sweep
    - [x] Light + dark mode contrast remains acceptable in step cards and highlighted actions. %<8589>
    - [x] Desktop + narrower screens avoid horizontal overflow in step shell and progress strip. %<8590>
    - [x] One keyboard-only run confirms CTA path is reachable without focus traps. %<8591>
...
- Exit criteria:
  - Completed: UXInspections A-D are done and follow-up issues are logged in `docs/strategies/x13.3_UX_ENGINEERING_PASS.md` and `docs/strategies/x13.3_UX_ENGINEERING_PASS2.md`.
