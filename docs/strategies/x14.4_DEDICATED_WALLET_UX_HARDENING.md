...
## 4.1 Update wallet settings copy %<8799>
1. [x] Update wallet settings copy to explicitly state that CryptoZing expects a dedicated account xpub for receives. %<8800>
2. [x] Explain that sharing the same account for receives elsewhere can cause false payment attribution. %<8801>
3. [x] Clarify that viewing or spending from that account elsewhere is fine. %<8802>
...
## 4.2 Reinforce the dedicated-account requirement in onboarding %<8803>
1. [x] Add onboarding reinforcement in the wallet step with a concise warning block and link to the Helpful Notes anchor. %<8804>
2. [x] Add an explicit acknowledgment checkbox only if Browser QA shows the warning copy is being ignored. %<8805>
...
## 4.3 Publish a Helpful Notes explainer for less technical users %<8806>
1. [x] Add a public Helpful Notes article that explains CryptoZing's watch-only model in plain language. %<8807>
2. [x] Explain why automatic payment tracking needs a dedicated receiving account key. %<8808>
3. [x] Explain that spending elsewhere is fine, but receiving elsewhere with the same account key makes automatic attribution unreliable. %<8809>
4. [x] Explain what unsupported configuration means and why the recommended fix is to connect a fresh dedicated account key. %<8810>
5. [x] Recommend separate receive and spend apps or accounts as the safest pattern without making separate apps a hard product requirement. %<8811>
...
## 4.4 Keep dedicated-wallet guidance usable and traceable %<8812>
1. [x] Apply `docs/UX_GUARDRAILS.md` so dedicated-wallet guidance does not introduce layout shift. %<8813>
2. [x] Preserve wallet input on validation failures. %<8814>
3. [x] Keep keyboard and focus behavior sane for any new guidance controls. %<8815>
4. [x] Add event or log entries when users save wallet settings after seeing dedicated-account guidance if support/debug traceability proves necessary. %<8816>
...
## 4.5 Verify Phase 4 %<8817>
Automated / command verification:
1. [x] Run `./vendor/bin/sail artisan test` at minimum for merge-ready Phase 4 work. %<8818>
   Current result on 2026-03-19: `236 passed`.
2. [x] Add or expand automated coverage for wallet-settings copy/flow changes, any onboarding reinforcement that ships, and any Helpful Notes linkage or rendering that ships with this phase. %<8819>
...
3. [x] Confirm any telemetry or logging added for guidance acknowledgment is emitted as expected. %<8820>
...
Browser QA:
4. [x] Verify dedicated-account warning clarity on wallet settings. %<8821>
...
5. [x] Verify onboarding reinforcement is understandable and does not introduce layout shift or focus regressions. %<8822>
...
6. [x] Verify the Helpful Notes explainer is understandable to a less technical audience and matches the in-app warning language. %<8823>
