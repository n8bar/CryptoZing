...
- When a donation is first seen — still unconfirmed or already confirmed — a notification email goes to a configured operator address, stating the payment's confirmation state truthfully; the donation itself settles only at the confirmation gate. Donors receive no mail.
