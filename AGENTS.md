...
- The content site lives in its own repo, [`n8bar/cryptozing-site`](https://github.com/n8bar/cryptozing-site) (split out in MS20.2); article work happens there, never in this repo.
...
- Outbound mail is restricted per environment at the provider, not by an app flag: prod sends from the custom Mailgun domain, which by design has no recipient restrictions, and every non-prod environment sends from the Mailgun **sandbox** domain, which refuses anyone not on its Authorized Recipients list with a 403. Never point a non-prod environment at the custom sending domain. Aliasing and the sandbox domain do not combine — aliasing rewrites each recipient to a generated catch-all address that can never be an authorized recipient, so enabling both refuses all mail.
